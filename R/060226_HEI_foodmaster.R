# 6/2/26, testing HEI on food_master (contains foodcode, fped).
# gtin-upc doesn't match up at all.

library(tidyverse)

# read in food_master
library(readr)
food_master <- read_csv("dataraw/food_master.csv")
...
compute_hei <- function(food_list, newdf) {

  diet <- food_list %>%
    left_join(newdf, by = "FOODCODE") %>%
    ...
    mutate(across(
      c(
        ...
        PF_MPS_TOTAL, PF_EGGS, PF_NUTSDS,
        PF_SOY, PF_LEGUMES,
  ...
  list(
    ...
    totals = totals,
    foods = diet %>%
      select(FOODCODE, DESCRIPTION, grams)
  )
}
...
hei_result <- compute_hei(food_list, food_master)
hei_result

hei_result$total_score

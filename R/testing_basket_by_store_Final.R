...
# Pull all unique fdc_ids seen across all stores
...
# Derived from branded_food filtered to store fdc_ids
df <- branded_food %>%
  filter(fdc_id %in% food_id) %>%
...
# 18. PER-STORE BASKET SIMULATION
# For each store:
...
# 20. Supplementary EDA of all store simulations
...
store_summary <- all_store_sims %>%
  ...
  summarise(
    ...
    n_baskets   = n(),
    pct_good    = mean(hei_total >= good_hei_threshold) * 100,  # % of that store's baskets scoring HEI scores > 80
    .groups = "drop"
  )
